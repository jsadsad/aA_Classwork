...
def large_countries
  # Show the names of the countries that have a population of 
  # at least
...
def high_population_gdps
  # Give the names and the per capita GDPs of countries with a 
  # population
...
def population_in_millions
  ...
  #with continent 'South America'. 
  #Divide the population by 1,000,000 to get
...
def name_and_population
  # Show the name and population for 'France', 'Germany', 
  # and 'Italy'
  execute(<<-SQL)
    SELECT
      name, population
...
def united_we_stand
  # Show the countries that have a name that includes the 
  #word 'United'
  execute(<<-SQL)
    SELECT
      name
