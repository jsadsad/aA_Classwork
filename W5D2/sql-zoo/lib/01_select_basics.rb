...
def per_capita_gdp
  # Show the name and per capita gdp (gdp/population) for 
  #each country where
  # the area is over 5,000,000 km^2
  execute(<<-SQL)
    SELECT 
      name, gdp/population as gdp_capita
    FROM 
      countries
    WHERE
      area > 5000000 
...
def small_and_wealthy
  # Show the name and continent of countries where the area is 
  #less than 2,000
...
def scandinavia
  # Show the name and the population for 'Denmark', 'Finland', 
  #'Norway', and
  # 'Sweden'
  execute(<<-SQL)
  SELECT
